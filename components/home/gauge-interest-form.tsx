...
/**
 * ⚠️ CANONICAL "Gauge My Interest" SECTION — DO NOT REPLACE WITH THE OLD CARD GRID.
 *
 * This is the ONE component the homepage renders for "What should we throw
 * next?". It is the FORM version (name / email / phone / instagram / idea
 * title / idea description / preferred vibe / consent → "Submit Idea").
 *
 * It must NOT be reverted to the old concept-card grid (Silent Disco,
 * Warehouse Rave, Day Party, Campus Takeover, Artist Pop-Up, Rooftop Game
 * Night). That grid and its data (components/public/concept-cards.tsx,
 * lib/content.ts EVENT_CONCEPTS) were removed on purpose.
 *
 * Backend: posts to the server route /api/interest, which writes to the
 * Airtable INTEREST FORMS table ONLY (never Supabase). The Airtable API key
 * stays server-side — this component only does a same-origin fetch.
 */
export function GaugeInterestForm() {
