...
/**
 * "What should we throw next?" idea form. Posts to /api/interest (Supabase
 * + optional Airtable). Collects contact info, the event idea, and consent.
 */
export function InterestForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      idea_title: fd.get('idea_title'),
      idea_description: fd.get('idea_description'),
      preferred_vibe: fd.get('preferred_vibe'),
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3" aria-label="Event idea form" noValidate>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <input name="name" required autoComplete="name" placeholder="Name *" aria-label="Name" className="field-input" />
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <input name="instagram" placeholder="@instagram (optional)" aria-label="Instagram handle" className="field-input" />
      </div>
      <div>
        <input name="idea_title" required placeholder="Your event idea (title) *" aria-label="Event idea title" className="field-input" />
      ...
      />
      <input name="preferred_vibe" placeholder="Preferred vibe / day (optional)" aria-label="Preferred vibe or day" className="field-input" />
      <label className="flex items-start gap-3 py-1 text-sm text-muted-foreground">
        <input type="checkbox" name="consent" className="mt-1 size-4 accent-[hsl(220_70%_28%)]" aria-label="Consent to be contacted" />
