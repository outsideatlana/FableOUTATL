...
export function HostedManager({ initialHosted }: { initialHosted: HostedRow[] }) {
  ...
  async function uploadLogo(): Promise<{ url: string; pathname: string }> {
    if (!file) return { url: form.image_url, pathname: form.image_pathname };
    const blob = await uploadImageToBlob({ file, folder: 'hosted' });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setStatus('');
    try {
      const { url: image_url, pathname: image_pathname } = await uploadLogo();
      const payload = {
        name: form.name,
      ...
      // Upload validation (type / 4.5MB) errors land here.
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_1.3fr] lg:items-start">
      ...
      <section className="border border-line bg-ink-800 p-6 lg:sticky lg:top-20">
        ...
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          ...
          <Field label="Logo / Image" htmlFor="h-img" hint="JPG, PNG, WEBP, or GIF · max 4.5MB" error={errors.image_url}>
            <input
              id="h-img"
              type="file"
      ...
      <section>
        ...
        {initialHosted.length === 0 ? (
          <p className="border border-line bg-ink-800 p-6 text-sm text-muted">
            No hosted entries yet. Add real artists/brands you&apos;ve worked with — the public section
