...
// POST /api/hosted/reorder — admin: persist new display order.
  ...
  try {
    ...
    await Promise.all(
      parsed.data.order.map((item) =>
        supabase.from('hosted').update({ sort_order: item.sort_order }).eq('id', item.id),
      ),
    );
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[hosted] reorder error:', err);
