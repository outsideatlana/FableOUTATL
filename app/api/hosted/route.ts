...
// GET /api/hosted — public: published "Who We've Hosted" entries.
export async function GET() {
  const hosted = await getPublicHosted();
  return NextResponse.json({ hosted });
}

// POST /api/hosted — admin: create an entry (image already uploaded to
// Supabase Storage via /api/upload; this stores url + path + metadata).
