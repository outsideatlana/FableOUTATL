...
// PATCH /api/hosted/:id — admin: update a hosted entry. Replaces the old
// blob if the image changed.
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    ...
    // Remove the superseded blob if the image was replaced/cleared.
    if (existing?.image_pathname && existing.image_pathname !== data.image_pathname) {
      await deleteFromBlob(existing.image_pathname);
    }
...
// DELETE /api/hosted/:id — admin: delete a hosted entry (and its blob).
